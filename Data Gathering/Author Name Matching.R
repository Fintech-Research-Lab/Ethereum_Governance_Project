...
# put a mark in twitter file that shows which twitter names are extra

tw1$extra_in_twitter[which(tw$mod_name %in% author_name$mod_name == FALSE)] <- "Extra in Twitter"
...
fwrite(tw2,"c:/Users/moazz/Box/Fintech Research Lab/Ethereum Governance Project/modified_twitter_data_6-2-2023.csv")
...
length(which(is.na(gh_matched_names) == TRUE)) # 139 names could not be matched using fuzzy logic jw method after treatment
...
# add extra in gh column in github file
g1$extra_in_gh <- ""
g1$extra_in_gh[which(! g1$mod_name %in% author_name$mod_name == TRUE)] <- "extra in GitHub"

g2 <- merge(g1,author_name[,c("mod_name","author_id","gh_matched_names")], by.x = "mod_name", by.y = "mod_name", all.x = TRUE)

# save modified gh data
fwrite(g2,"c:/Users/moazz/Box/Fintech Research Lab/Ethereum Governance Project/modified_github_data_6-2-2023.csv")
...
# scratch work
dist_matrix <- stringdistmatrix(author_unique$mod_name, tw$mod_name, method = "jw")# The purpose of this code is to create a name matching algorithm using Fuzzy Logic
# We have four datasets that are organized by authors but there is no unique alphanumeric
# id that combines these data. This code is designed to do the following
